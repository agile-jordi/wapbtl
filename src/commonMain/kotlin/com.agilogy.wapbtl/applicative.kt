...
fun <A, B, Z> Arb.Companion.product2(
    ...
    Arb { random -> f(a.generate(random), b.generate(random)) }

fun <A, B, C, Z> Arb.Companion.product3(
    a: Arb<A>, b: Arb<B>, c: Arb<C>, f: (A, B, C) -> Z
): Arb<Z> =
    Arb { random -> f(a.generate(random), b.generate(random), c.generate(random)) }

fun <A, B, C, D, Z> Arb.Companion.product4(
    a: Arb<A>, b: Arb<B>, c: Arb<C>, d: Arb<D>, f: (A, B, C, D) -> Z
): Arb<Z> =
    Arb { random -> f(a.generate(random), b.generate(random), c.generate(random), d.generate(random)) }